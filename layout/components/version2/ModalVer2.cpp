#include "./ModalVer2.hpp"

std::vector<std::unique_ptr<Component>> ModalVer2::modals;

ModalVer2::ModalVer2(const sf::RenderWindow &renderWindow, int width, int height) {
...
auto ModalVer2::getPosition() const -> sf::Vector2f {
    return modalBase[0].position;
}

auto ModalVer2::getSize() const -> sf::Vector2f {
    return {modalBase[1].position.x - modalBase[0].position.x, modalBase[2].position.y - modalBase[1].position.y};
}

auto ModalVer2::setPosition(sf::Vector2f const &position) -> void {
...
auto ModalVer2::setSize(const sf::Vector2f &size) -> void {
...
auto ModalVer2::isMouseOver(const sf::Vector2i &mousePosition) const -> bool {
    return ComponentUtil::isMouseOver(*this, mousePosition);
}

auto ModalVer2::show() -> void {
    modals.push_back(std::make_unique<ModalVer2>(*this));
}

auto ModalVer2::hide() -> void {
...
auto ModalVer2::draw(sf::RenderWindow &renderWindow) -> void {
    renderWindow.draw(modalBase);
}

auto ModalVer2::setVerticalGradient(const sf::Color &upperColor, const sf::Color &bottomColor) -> void {
...
auto ModalVer2::setHorizontalGradient(const sf::Color &leftColor, const sf::Color &rightColor) -> void {
...
auto ModalVer2::setInlineColor(const sf::Color &color) -> void {
